...
class ImageStack(QObject):
    ...
    def __init__(self,
        ...
        self._pixmap_cache = CachedData(None)
    ...
    def qimage(self, crop_to_bounds: bool = True) -> QImage:
        """Returns combined visible layer content as a QImage object, optionally including unsaved layers."""
        if self._image_cache.valid and not crop_to_bounds:
    ...
    def pixmap(self) -> QPixmap:
        ...
        if self._pixmap_cache.valid:
            return self._pixmap_cache.data
        image = self.qimage()
        new_pixmap = QPixmap.fromImage(image)
        self._pixmap_cache.data = new_pixmap
        return new_pixmap
    ...
    def get_color_at_point(self, image_point: QPoint) -> QColor:
        ...
        if not 0 <= image_point.x() < self.size.width() or not 0 <= image_point.y() < self.size.height():
            return QColor(0, 0, 0)
        return self.qimage().pixelColor(image_point)
    ...
    def copy_layer(self, layer: Optional[ImageLayer | int] = None) -> None:
        ...
        def _add_copy(copy=layer_copy, idx=layer_index + 1):
            self._insert_layer_internal(copy, idx)

        def _remove_copy(copy=layer_copy):
            self._remove_layer_internal(copy)

        commit_action(_add_copy, _remove_copy)
    ...
    def merge_layer_down(self, layer: Optional[ImageLayer | int] = None) -> None:
        ...
        base_layer_image = base_layer.image
        ...
        painter.drawImage(QRect(top_layer.position - merged_bounds.topLeft(), top_layer.size), top_layer.image)

        def _do_merge() -> None:
            self._remove_layer_internal(top_layer, True)
            base_layer.set_image(merged_image)
            base_layer.set_position(merged_bounds.topLeft())
            if top_layer.visible != base_layer.visible:
                self.content_changed.emit()

        def _undo_merge(insert_index=top_layer_index) -> None:
            base_layer.set_image(base_layer_image)
            base_layer.set_position(base_pos)
            self._insert_layer_internal(top_layer, insert_index, True)
    ...
    def set_image(self, image_data: Image.Image | QImage | QPixmap):
        ...
        def _undo_load():
            ...
            self.selection_layer.set_image(selection_image)
    ...
    def _set_size(self, new_size: QSize) -> None:
        ...
        self._set_generation_area_internal(self._generation_area)
        self.selection_layer.set_size(new_size, False)

    def _invalidate_all_cached(self,) -> None:
    ...
    def _create_layer_internal(self, layer_name: Optional[str] = None,
                               image_data: Optional[Image.Image | QImage | QPixmap] = None) -> ImageLayer:
    ...
    def _insert_layer_internal(self, layer: ImageLayer, index: int, connect_signals=True, empty_image=False) -> None:
        ...
        assert layer not in self._layers, f'layer {layer.name} is already in the image stack.'
    ...
    def _remove_layer_internal(self, layer: ImageLayer, disconnect_signals=True) -> None:
        ...
        if next_active_layer != self.active_layer:
            if next_active_layer is None:
                self._active_layer_id = None
            else:
                self._active_layer_id = next_active_layer.id
