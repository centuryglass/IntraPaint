"""Initializes a libmypaint surface and renders it within a QGraphicsScene."""
import math
...
class MyPaintSceneSurface(QObject):
    ...
    tile_created = Signal(MyPaintSceneTile)
    tile_updated = Signal(MyPaintSceneTile)
    ...
    def __init__(self, size: QSize) -> None:
        ...
        self._tiles: dict[str, MyPaintSceneTile] = {}
        self._tile_buffer: Any = None

        self._null_buffer = TilePixelBuffer()
        self._null_tile = MyPaintSceneTile(self._null_buffer)
    ...
    def load_image(self, image: QImage) -> None:
        ...
        for y in range(self._tiles_height):
            for x in range(self._tiles_width):
                ...
                x_px = x * TILE_DIM
                y_px = y * TILE_DIM
                if MyPaintSceneTile.copy_image_into_pixel_buffer(pixel_buffer, image, x_px, y_px, True):
    ...
    def get_tile_from_idx(self, x: int, y: int, clear_buffer_if_new: bool = True) -> MyPaintSceneTile:
        ...
        if point in self._tiles:
            ...
            tile = MyPaintSceneTile(pixel_buffer, clear_buffer_if_new, QSize(width, height))
