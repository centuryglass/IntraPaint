"""
Combines various canvas modules to represent and control an edited image section.
"""
...
class CanvasView(FixedAspectGraphicsView):
    """
    ...
    class DrawMode:
        ...
        def is_valid(option):
            ...
            return option == CanvasView.DrawMode.MASK or option == CanvasView.DrawMode.SKETCH or option is None
    ...
    class ToolMode:
        ...
        def is_valid(option):
            ...
            return option == CanvasView.ToolMode.PEN or option == CanvasView.ToolMode.ERASER or option is None
    ...
    def __init__(self,
        ...
        self._tool_mode = CanvasView.ToolMode.PEN
        self._draw_mode = CanvasView.DrawMode.MASK if mask_canvas.enabled else CanvasView.DrawMode.SKETCH
        ...
        def update_image() -> None:
            ...
            if isinstance(sketch_canvas, SketchCanvas):
                self.background = layer_stack.qimage_selection_content()
            else:
                self.background = get_transparency_tile_pixmap(self.content_size)
    ...
    def set_draw_mode(self, mode: str) -> None:
        ...
        Parameters
        ----------
        mode : str from CanvasView.DrawMode
            Selected drawing mode.
        """
        if mode == self._draw_mode:
            return
        if not CanvasView.DrawMode.is_valid(mode):
            raise ValueError(f'tried to set invalid drawing mode {mode}')
        if mode == CanvasView.DrawMode.MASK and not self._mask_canvas.enabled:
            raise ValueError('called setDrawMode(MASK) when mask mode is disabled')
        if mode == CanvasView.DrawMode.SKETCH and not self._sketch_canvas.enabled:
            raise ValueError('called setDrawMode(SKETCH) when sketch mode is disabled')
        self._draw_mode = mode
        if self._draw_mode != mode:
            self._draw_mode = mode
            self._mask_canvas.setOpacity(0.4 if mode == CanvasView.DrawMode.SKETCH else 0.6)

    def get_draw_mode(self) -> Optional[str]:
        """Returns the current CanvasView.DrawMode drawing mode."""
    ...
    def set_tool_mode(self, mode: ToolMode):
        ...
        if mode == self._tool_mode:
            return
        if not CanvasView.ToolMode.is_valid(mode):
            raise ValueError(f'tried to set invalid drawing tool mode {mode}')
        self._tool_mode = mode

    def get_tool_mode(self) -> str:
        """Returns the current CanvasView.ToolMode drawing tool mode."""
        return self._tool_mode

    def clear(self):
        """Clears image content in the active canvas."""
        if self._draw_mode == CanvasView.DrawMode.SKETCH:
    ...
    def undo(self):
        ...
        canvas = self._sketch_canvas if self._draw_mode == CanvasView.DrawMode.SKETCH else self._mask_canvas
        if canvas.enabled:
            canvas.undo()
        self.update()
    ...
    def get_color_at_point(self, point: QPoint) -> QColor:
        ...
        If the point is outside the widget bounds, QColor(0, 0, 0) is returned instead.
        """
        ...
        def get_component(sketch_component: float, image_component: float) -> float:
            """Get combined values for an RGBA image color component"""
    ...
    def mousePressEvent(self, event: Optional[QMouseEvent]) -> None:
        ...
        if event.button() == Qt.LeftButton or event.button() == Qt.RightButton:
            ...
            if key_modifiers == Qt.ControlModifier and self._draw_mode == CanvasView.DrawMode.SKETCH:
                point = self.widget_to_scene_coordinates(event.pos()).toPoint()
                color = self.get_color_at_point(point)
                if color is not None:
                    self.color_selected.emit(color)
            else:
                canvas = self._sketch_canvas if self._draw_mode == CanvasView.DrawMode.SKETCH else self._mask_canvas
                color = QColor(self._sketch_color if self._draw_mode == CanvasView.DrawMode.SKETCH else Qt.red)
                if self._draw_mode == CanvasView.DrawMode.SKETCH and self._pressure_opacity:
                ...
                if self._line_mode:
                    new_point = self.widget_to_scene_coordinates(event.pos()).toPoint()
                    ...
                    if self._tool_mode == CanvasView.ToolMode.ERASER:
                    ...
                    self._last_point = self.widget_to_scene_coordinates(event.pos()).toPoint()
                    if self._tool_mode == CanvasView.ToolMode.ERASER or self._tablet_eraser:
    ...
    def mouseMoveEvent(self, event: Optional[QMouseEvent]) -> None:
        ...
        if (Qt.LeftButton == event.buttons() or Qt.RightButton == event.buttons()) and self._drawing:
            ...
            new_last_point = self.widget_to_scene_coordinates(event.pos()).toPoint()
