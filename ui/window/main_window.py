...
from ui.config_control_setup import connected_textedit, connected_spinbox, connected_checkbox
...
class MainWindow(QMainWindow):
    ...
    def __init__(self,
        ...
        controller : controller.base_controller.stable_diffusion_controller.BaseController child class object
        ...
        add_action('Undo', 'Ctrl+Z', lambda: if_not_selecting(self._mask_panel.undo), edit_menu)
        add_action('Redo', 'Ctrl+Shift+Z', lambda: if_not_selecting(self._mask_panel.redo), edit_menu)
                ...
                if 'lcm-lora-sdv1-5' in loras:
                    def set_lcm_mode():
                        ...
                        if lora_key not in prompt:
                            config.set(Config.PROMPT, f'{prompt} {lora_key}')
                        config.set(Config.GUIDANCE_SCALE, 1.5)
                        config.set(Config.SAMPLING_STEPS, 8)
                        config.set(Config.SAMPLING_METHOD, 'LCM')
    ...
    def _build_control_layout(self, controller):
        ...
        text_prompt_textbox = connected_textedit(inpaint_panel, self._config, Config.PROMPT)
        negative_prompt_textbox = connected_textedit(inpaint_panel, self._config, Config.NEGATIVE_PROMPT)

        batch_size_spinbox = connected_spinbox(inpaint_panel, self._config, Config.BATCH_SIZE)

        batch_count_spinbox = connected_spinbox(inpaint_panel, self._config, Config.BATCH_COUNT)
        ...
        guidance_scale_spinbox = connected_spinbox(inpaint_panel, self._config, Config.GUIDANCE_SCALE)

        skip_steps_spinbox = connected_spinbox(inpaint_panel, self._config, Config.SKIP_STEPS)
        ...
        upscale_mode_label.setText(self._config.get_label(Config.UPSCALE_MODE))
        upscale_mode_list = self._create_scale_mode_selector(inpaint_panel, Config.UPSCALE_MODE)
        downscale_mode_label = QLabel(inpaint_panel)
        downscale_mode_label.setText(self._config.get_label(Config.DOWNSCALE_MODE))
        downscale_mode_list = self._create_scale_mode_selector(inpaint_panel, Config.DOWNSCALE_MODE)

        more_options_bar.addWidget(QLabel(inpaint_panel, text=self._config.get_label(Config.GUIDANCE_SCALE)),
                stretch=0)
        more_options_bar.addWidget(guidance_scale_spinbox, stretch=20)
        more_options_bar.addWidget(QLabel(inpaint_panel, text=self._config.get_label(Config.SKIP_STEPS)), stretch=0)
        ...
        layout.addWidget(QLabel(inpaint_panel, text=self._config.get_label(Config.NEGATIVE_PROMPT)), 2, 1, 1, 1)
        layout.addWidget(negative_prompt_textbox, 2, 2, 1, 1)
        layout.addWidget(QLabel(inpaint_panel, text=self._config.get_label(Config.BATCH_SIZE)), 1, 3, 1, 1)
        layout.addWidget(batch_size_spinbox, 1, 4, 1, 1)
        layout.addWidget(QLabel(inpaint_panel, text=self._config.get_label(Config.BATCH_COUNT)), 2, 3, 1, 1)
