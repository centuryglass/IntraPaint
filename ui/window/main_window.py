...
from PyQt5.QtWidgets import QMainWindow, QGridLayout, QLabel, QWidget, QPushButton, QVBoxLayout, QHBoxLayout, \
    QLayout, QComboBox, QFileDialog, QStackedWidget, QAction, QMenuBar, QBoxLayout
...
from data_model.image.filled_canvas import FilledMaskCanvas
...
class MainWindow(QMainWindow):
    ...
    def __init__(self,
        ...
        def if_not_selecting(fn: Callable[[None], None]) -> None:
    ...
    def _setup_correct_layout(self) -> None:
        """Applies whatever layout is most appropriate given the current window dimensions."""
    ...
    def _build_control_layout(self, controller) -> None:
        ...
        more_options_bar.addWidget(QLabel(inpaint_panel, text=self._config.get_label(AppConfig.GUIDANCE_SCALE)),
                                   stretch=0)
        more_options_bar.addWidget(guidance_scale_spinbox, stretch=20)
        more_options_bar.addWidget(QLabel(inpaint_panel, text=self._config.get_label(AppConfig.SKIP_STEPS)), stretch=0)
        ...
        layout = QGridLayout()
        layout.addWidget(QLabel(inpaint_panel, text=self._config.get_label(AppConfig.PROMPT)), 1, 1, 1, 1)
        layout.addWidget(text_prompt_textbox, 1, 2, 1, 1)
        layout.addWidget(QLabel(inpaint_panel, text=self._config.get_label(AppConfig.NEGATIVE_PROMPT)), 2, 1, 1, 1)
        layout.addWidget(negative_prompt_textbox, 2, 2, 1, 1)
        layout.addWidget(QLabel(inpaint_panel, text=self._config.get_label(AppConfig.BATCH_SIZE)), 1, 3, 1, 1)
        layout.addWidget(batch_size_spinbox, 1, 4, 1, 1)
        layout.addWidget(QLabel(inpaint_panel, text=self._config.get_label(AppConfig.BATCH_COUNT)), 2, 3, 1, 1)
    ...
    def set_sample_selector_visible(self, visible: bool):
        ...
        if visible == is_visible:
            return
        if visible:
            mask = self._mask if (self._config.get(AppConfig.EDIT_MODE) == 'Inpaint') else FilledMaskCanvas(self._config)
