...
try:
    from ui.panel.brush_picker import BrushPicker
except ImportError:
    BrushPicker = None
...
class MaskPanel(QWidget):
    ...
    def __init__(self,
        ...
        config.connect(self, AppConfig.MASK_BRUSH_SIZE, lambda s: update_brush_size(MaskCreator.DrawMode.MASK, s))
        config.connect(self, AppConfig.SKETCH_BRUSH_SIZE, lambda s: update_brush_size(MaskCreator.DrawMode.SKETCH, s))
        ...
        def set_drawing_tool(selection: MaskCreator.ToolMode):
            """Sets the active tool in the mask creator."""
        ...
        self._fill_mask_button.clicked.connect(self._mask_creator.fill)
    ...
    def tabletEvent(self, unused_tablet_event: Optional[QTabletEvent]) -> None:
        ...
        if self._pressure_size_checkbox is None:
            ...
            config.connect(self, AppConfig.PRESSURE_SIZE, self._mask_creator.set_pressure_size_mode)
            self._mask_creator.set_pressure_size_mode(config.get(AppConfig.PRESSURE_SIZE))

            self._pressure_opacity_checkbox = connected_checkbox(self, config, AppConfig.PRESSURE_OPACITY)
            config.connect(self, AppConfig.PRESSURE_OPACITY, self._mask_creator.set_pressure_opacity_mode)
            self._pressure_opacity_checkbox.setIcon(QIcon(QPixmap(RESOURCES_PRESSURE_OPACITY_PNG)))
            self._mask_creator.set_pressure_opacity_mode(config.get(AppConfig.PRESSURE_OPACITY))
    ...
    def set_draw_mode(self, mode: Optional[str], ignore_if_unchanged: bool = True):
        ...
        self._brush_size_slider.connect_key(AppConfig.MASK_BRUSH_SIZE if mode == MaskCreator.DrawMode.MASK
    ...
    def toggle_draw_mode(self) -> None:
        ...
        self.set_draw_mode(MaskCreator.DrawMode.MASK
                           if self._mask_creator.get_draw_mode() == MaskCreator.DrawMode.SKETCH
                           else MaskCreator.DrawMode.SKETCH)

    def get_brush_size(self) -> int:
        """Returns the current brush size in pixels."""
        return self._config.get(AppConfig.MASK_BRUSH_SIZE
                                if self._mask_creator.get_draw_mode() == MaskCreator.DrawMode.MASK
                                else AppConfig.SKETCH_BRUSH_SIZE)

    def set_brush_size(self, size: int) -> None:
        """Sets the current brush size in pixels."""
        self._config.set(AppConfig.MASK_BRUSH_SIZE if self._mask_creator.get_draw_mode() == MaskCreator.DrawMode.MASK
    ...
    def paintEvent(self, event: Optional[QPaintEvent]) -> None:
        ...
        if not self._color_picker_button.isHidden():
            painter.setPen(
                QPen(self._mask_creator.get_sketch_color(), self._border_size // 2, Qt.SolidLine, Qt.RoundCap,
                     Qt.RoundJoin))
    ...
    def _update_brush_cursor(self):
        ...
        if scaled_size <= 10:
            ...
            if scaled_size > 500:
                ...
                # TODO: render a pseudo-cursor in the graphics view insstead of actually changing the cursor, at least
